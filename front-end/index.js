...
window.addEventListener('load', () => {
    ...
    axios.get('http://localhost:8000/products').then((products) => {
...
document.addEventListener('click',(e)=>{
    ...
    if (e.target.className=='shop-item-button'){
        ...
        axios.post('http://localhost:8000/cart', { productId: prodId}).then(data => {
            if(data.data.error){
    ...
    if (e.target.className=='cart-btn-bottom' || e.target.className=='cart-bottom' || e.target.className=='cart-holder'){
        axios.get('http://localhost:8000/cart').then(carProducts => {
...
function deleteCartItem(e, prodId){
    e.preventDefault();
    axios.post('http://localhost:8000/cart-delete-item', {productId: prodId})
